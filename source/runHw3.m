...
param = 1.5;
blended_result = blendImagePair(fish, fish_mask, horse, horse_mask,'blend', param);
figure, imshow(blended_result);
imwrite(blended_result, 'blended_result.png');
overlay_result = blendImagePair(fish, fish_mask, horse, horse_mask, 'overlay', param);
...
% Your own panorama
img_left = im2single(imread('Watson_left.jpg'));
img_right = im2single(imread('Watson_right.jpg'));
img_left = imresize(img_left,0.25);
img_right = imresize(img_right,0.25);

stitched_img = stitchImg(img_right, img_left);
figure, imshow(stitched_img);
imwrite(stitched_img, 'Watson_panorama.jpg');
%--------------------------------------------------------------------------
